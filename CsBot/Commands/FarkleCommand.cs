...
{
	class FarkleCommand : IIrcCommand
	{
		class FarkleUsers
		{
			string User { get; set; }
			bool FarkleToken { get; set; }
			int FarkleValue { get; set; }
			bool FarkleFlag { get; set; }
		}

		readonly List<FarkleUsers> Players = new List<FarkleUsers> ();

		static int DICE = 6;
		static bool FarkleInSession;
		readonly Dictionary<int, string> FarkleMembers = new Dictionary<int, string> ();
		static int NumOfFarkleMembers;
		static int DiceToThrow = 6;
		readonly Dictionary<int, int> dice = new Dictionary<int, int> ();
		static int FarkleTotal;
		static int TempFarkleTotal;
		readonly Random random = new Random ();

		public FarkleCommand ()
		{
		...
		public void FarkleFarkle()
		{
			...
			if (command.Length == endCommand + 1) {
				...
				string name = "";
				bool found = false;
				foreach (string user in FarkleMembers.Values) { //Get the next user in the list
					if (found) {
						name = user;
						return;
					}
		...
		bool FarkleIsThreePairs (Dictionary<int, int> dice)
		{
			if (dice.Count != 3)
				return false;
		...
		int FarkleValueCheck (Dictionary<int, int> dice)
		{
			int returnValue = 0;
			foreach (int die in dice.Keys) {
				if (die == 1) {
